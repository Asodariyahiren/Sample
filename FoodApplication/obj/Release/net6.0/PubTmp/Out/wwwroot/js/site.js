...
async function cart() {
    ...
    if ($(iTag).hasClass('fa-regular')) {
        ...
        if (result.data && result.data.recipe) {
            ...
            delete cart.id;
            cardRequest(cart, "SaveCart", "fa-solid", "fa-regular", iTag, false);
        } else {
        ...
        cardRequest(data, "RemoveCartFromList", "fa-regular", "fa-solid", iTag, false);
    }
}

function cardRequest(data, action, addcls, removecls, iTag, isReaload) {
    $.ajax({
        url: '/Cart/' + action,
        type: 'POST',
        data: data,
...
function removecartfromlist(id) {
    let data = { Id: id };
    cardRequest(data, "RemoveCartFromList", null, null, null, true);
}